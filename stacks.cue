...
#gradle: #advanced & {
	...
	layers: {
		sayt: *([ { files: [ ".mise.toml", "mise.lock" ], from: list.Concat([[ "root_sayt", "root_gradle" ], [ for s in copy { "\(s._prefix)_sources" } ]]) } ]) | [ ...docker.#run ]
...
#pnpm: #advanced & {
	...
	layers: {
				sayt: *([ { files: [ ".mise.toml", "mise.lock" ], from: list.Concat([[ "root_sayt", "root_pnpm" ], [ for s in copy { "\(s._prefix)_sources" } ]]) } ]) | [ ...docker.#run ]
